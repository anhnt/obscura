...
DISABLE_CONSOLE_FLAG=""

if [ "$DISABLE_CONSOLE" = "true" ] || [ "$DISABLE_CONSOLE" = "1" ]; then
    DISABLE_CONSOLE_FLAG="--disable-console"
fi

OBSCURA_PORT=$((PORT + 10000))

echo "Starting Obscura on local port $OBSCURA_PORT (Workers: $WORKERS)..."
obscura $DISABLE_CONSOLE_FLAG serve --port $OBSCURA_PORT --workers $WORKERS $STEALTH_FLAG &
